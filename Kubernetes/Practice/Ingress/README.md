...
```bash
echo "$(minikube ip) so.com" | sudo tee -a /etc/hosts
```
<center>OR</center>
...
<b>11) Now, test the routing :</b>
  ...
  - curl http://so.com/apache to access the Apache service.
  
  ```yml
  curl http://so.com/apache
  ```
  - curl http://so.com/nginx to access the NGINX service.
  
  ```yml
  curl http://so.com/nginx
  ```
<center>OR</center>
